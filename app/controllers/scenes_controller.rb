class ScenesController < ApplicationController
  ...
  def index
    ...
    @scenes = Scene.order(title: :asc).offset(page_limit * @current_page).limit(page_limit)
    @next_page = @current_page + 1 if (Scene.count > page_limit * @current_page + page_limit)
    # @scenes = Scene.order(title: :asc).all
  end
